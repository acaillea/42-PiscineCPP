...
/*   Updated: 2022/09/20 17:22:14 by acaillea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::signForm( Form& fofo ) const
{
	...
	{
		fofo.beSigned( *this );
		std::cout << this->_name << " signed " << fofo.getName() << " form with grade " << this->_grade<< std::endl; 
		fofo.setSigned( 1 );
	}
	catch( const std::exception& e )
	{
		std::cout << "Bureaucrat " << this->_name << " With a Grade " << \
			this->_grade << " Cannot Sign " << fofo.getName() << " Because " << e.what() << std::endl;
	}
	return ;
}
